## code from https://github.com/NVIDIA/pix2pixHD/blob/master/models/networks.py
...
class MultiscaleDiscriminator(nn.Module):
    def __init__(self, input_nc=3, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d,
        ...
        self.num_l = 3 if use_sigmoid else 2

        for i in range(num_D):
            netD = NLayerDiscriminator(input_nc, ndf, n_layers, norm_layer, use_sigmoid, getIntermFeat)
            if getIntermFeat:
                for j in range(n_layers+self.num_l):
    ...
    def forward(self, input):
        ...
        for i in range(num_D):
            if self.getIntermFeat:
                model = [getattr(self, 'scale'+str(num_D-1-i)+'_layer'+str(j)) for j in range(self.n_layers+self.num_l)]
            else:
...
class FeatureMatchingLoss(nn.Module):
    def __init__(self, n_layers_D=3, num_D=3):
        super().__init__()
